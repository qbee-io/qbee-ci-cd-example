...
class InputReader:
    ...
    def replace_package_name(self):
        self.is_ready()
        if(not self.jsonData):
            print("call get_config before using replace_package_name in InputReader")
            sys.exit(1)
        current_pkg = os.path.basename(self.settings["package_name"])
        parts = current_pkg.split("_")
        if len(parts) != 3:
            print("invalid package name provided")
            sys.exit(1)
        basename = parts[0]
        for idx, item in enumerate(self.jsonData['items']):
            pkg_name = item['package']
            if pkg_name.endswith('.deb') and basename in os.path.basename(pkg_name):
                dirname = os.path.dirname(pkg_name)
                self.jsonData['items'][idx]["package"] = os.path.join(dirname, current_pkg)
                break
...
class ApiCalls:
    base_url = "{}/api/v2/".format(os.getenv("QBEE_ENDPOINT",default="https://www.app.qbee.io"))
    change_api = "change"
    commit_api = "commit"
    config_api = "config/"
